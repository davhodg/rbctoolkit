 ...
 * rbcBusy.c --
 *
 *      This module implements busy windows for the rbc toolkit.
 *
...
#define BUSY_THREAD_KEY	"Rbc Busy Data"
    ...
    Tk_Window tkRef; /* Window hosting the busy window */
{
    ...
    busyPtr = RbcCalloc(1, sizeof(Busy));
    assert(busyPtr);
    x = y = 0;
    ...
    for (tkChild = Rbc_FirstChild(tkParent); tkChild != NULL;
            tkChild = Rbc_NextChild(tkChild)) {
...
#else
        parent = Rbc_GetParent(Tk_Display(tkRef), Tk_WindowId(tkRef));
#endif
    } else {
    ...
    char **argv;
{
    ...
    Rbc_SetBooleanResult(interp, busyPtr->isBusy);
    Tcl_Release(busyPtr);
    return TCL_OK;
}
    ...
    Tcl_Interp *interp;
{
    ...
    if (dataPtr == NULL) {
        dataPtr = (BusyInterpData *) ckalloc(sizeof(BusyInterpData));
        assert(dataPtr);
