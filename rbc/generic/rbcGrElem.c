    ...
    Rbc_Chain *palette;
{
    ...
    if (linkPtr != NULL) {
        ...
        for (linkPtr = Rbc_ChainNextLink(linkPtr); linkPtr != NULL;
                linkPtr = nextPtr) {
    ...
    Element *elemPtr;
{
    ...
    for (i = 0; i < nWeights; i++) {
        for (linkPtr = Rbc_ChainLastLink(elemPtr->palette); linkPtr != NULL;
    ...
    Graph *graphPtr;
{
    ...
    for (linkPtr = Rbc_ChainFirstLink(graphPtr->elements.displayList);
        ...
        if ((Rbc_VectorNotifyPending(elemPtr->x.clientId)) ||
